      ...
      <div className="flex flex-col gap-8">
        {testimonials.map((t, idx) => (
          <motion.div
            ...
            className="flex flex-col md:flex-row gap-6 p-8 border-l-2 border-surface hover:border-glow-green transition-colors bg-[#0D0D0D]/50"
          >
            <div className="md:w-1/3 flex flex-col justify-between">
              <div>
                <div className="font-mono text-xs text-glow-green mb-1 animate-[pulse_3s_infinite]">{t.status}</div>
                <h3 className="font-sans font-bold text-primary">{t.client}</h3>
